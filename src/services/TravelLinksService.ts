/**
 * TravelLinksService — search/deep-link helpers (not full OTA checkout).
 */
...
class TravelLinksService {
  ...
  activitySearch(input: {city: string; query?: string}): TravelLink[] {
  ...
  async open(link: TravelLink): Promise<void> {
    log('TravelLinksService: open', link.id, link.url);
    await Linking.openURL(link.url);
  }
}

export default new TravelLinksService();
